...
exports.list = (req,res) => { //리스트 모듈
	...
	sql = ` SELECT  count(*) cnt FROM atr_board WHERE board_code = ? ${where} `;
	conn.query(sql,[body.board_code],(err,data)=>{
		...
		sql = ` SELECT * FROM atr_board WHERE board_code = ? ${where} ORDER BY num DESC LIMIT ?, ? `; 
...
exports.add = (req,res) => { //등록 프로세스 모듈
	...
	sql = " INSERT INTO  atr_board (board_code, subject, cont, id, regdate) values (?, ?, ?, ?,now()) ";
	conn.query(sql,
		[body.board_code
		, body.subject
		, body.cont
